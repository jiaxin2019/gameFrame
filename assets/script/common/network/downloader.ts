...
export class Downloader {
    ...
    private _downloadedMap: Map<string, DownloadTask> = null!;       //已下载资源列表，url -> DownloadTask
    private _downloadingMap: Map<string, DownloadItem[]> = null!;   //下载同一个url的item列表

    private constructor() {
        if (!JSB) {
            console.warn('Downloader is a NATIVE ONLY feature.');
        }
    ...
    downloadFile(item: DownloadItem | undefined) {
        if (!item) {
            return;
        }
        ...
        console.log(`Downloader, downloadFile, url=${requestURL}, downloadingCnt=${this._downloadingCnt}`);
    }
    ...
    private downloadQueueItem() {
        if (!this._downloadQueue || this._downloadQueue.length < 1) {
            console.log(`Downloader, no queue or all queue item had been downloaded`);
            return;
        }
    ...
    private onFileTaskSuccess(requestURL: string, jsbDownloader: any, task: DownloadTask) {
        //保存下载后的地址
        ...
        this._downloadingCnt--;
        console.log(`Downloader, onFileTaskSuccess, url=${requestURL}, downloadingCnt=${this._downloadingCnt}`);
        this.downloadQueueItem();
    }

    private onTaskError(requestURL: string, jsbDownloader: any, task: DownloadTask, errCode: number, errCodeInternal: number, errStr: string) {
        if (this._downloadingMap) {
            const downloadingItems = this._downloadingMap.get(requestURL);
            if (!downloadingItems) return;
            downloadingItems.forEach(item => {
        ...
        this.pushJsbDownloader(jsbDownloader);
        this._downloadingCnt--;
        console.log(`Downloader, onTaskError, url=${requestURL}, downloadingCnt=${this._downloadingCnt}`);
        this.downloadQueueItem();
    }
    ...
    private pushJsbDownloader(jsbDownloader: any) {
